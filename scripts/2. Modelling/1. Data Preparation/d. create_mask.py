...
from utilities import path_prep_annot_target, path_clean_mask
...
for class_type in classes:
    annots[class_type] = json.load(open([os.path.join(path_prep_annot_target,
                                                        class_type,
                                                        file)
                                            for file in os.listdir(os.path.join(path_prep_annot_target,
                                                                                class_type))
...
for class_type, annotations in annots.items():
    ...
    for annotation in annotations:
        ...
        try:
            os.remove(os.path.join(path_clean_mask, f"{annotation['metadata']['img_name'].split('.')[0]}.png"))
        except FileNotFoundError:
            continue
        # save the mask image
        Image.fromarray(b_mask).save(os.path.join(path_clean_mask, f"{annotation['metadata']['img_name'].split('.')[0]}_mask.png"))
